...
df1<- ysp %>% filter(product_category=="PUDO") %>% rename(pudo = gm) %>% select(store_name,pudo)
df2<- ysp %>% filter(product_category=="Sazka") %>% rename(sazka = gm) %>% select(store_name,sazka)                       
df<- left_join(df1,df2,by="store_name")                                                                                  

ggplot(df, aes(x = sazka, y = pudo)) +
  geom_point() +
  theme_bw() +
  geom_smooth(method="loess" , formula = y ~ x )+
  labs( title = "Pattern of association between PUDO and Gambling Gross Margins",
        y = "PUDO Gross Margin (100K)",
        x = "Gambling Gross Margin (100K)")
